...
После первого деплоя выполните миграции **один раз** (через Railway CLI или локально с `DATABASE_URL` от Railway):

```bash
railway run alembic upgrade head
```

Или через **one-off command** в Railway Dashboard (если доступно).

## 4. Web-сервис

Основной сервис запускает `server.py` — отдаёт `200 OK` на `/` и `/health`.  
Railway автоматически назначит порт через `PORT`.

## 5. Cron Job — сбор данных

Чтобы собирать данные каждые 15 минут:
